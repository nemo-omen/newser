...
// UI view functions
import { ArticleList, Default, SearchEmpty, SearchError } from "./views.js";
  ...
  function () {
    ...
    const createAppModel = function (oldState) {
      ...
      if (typeof oldState === "string") {
        ...
        state.articles = async () =>
          await search(config.defaultSearchTerm).articles;
    ...
    const search = async function (term) {
      ...
      if (term.length < 1) {
        // return;
        term = config.defaultSearchTerm;
      }

      // make fetch call
      const response = await fetch(
        `${config.apiUrl}${term}&lang=${config.apiLang}`,
        config.fetchOptions,
      );
    ...
    (function app() {
      ...
      const renderPageState = async function (data) {
        // sort parsed article data by date
        console.log({ data });
        if (data === undefined || data.length < 1) {
          // load empty template
          const fetched = await search(config.defaultSearchTerm);
          data = fetched.articles;
        }
        // loadView("articles", {
        //   // articles: async () => await search("The Last of Us").articles,
        //   articles: data,
        // });
        ...
        document.querySelectorAll(".bookmark-button").forEach(
          function (button) {
      ...
      schemeToggleButton.addEventListener("click", function () {
        ...
        schemeToggleButton.innerHTML = getColorSchemeButton(
          appModel.getScheme(),
        );
        updateApp();
      });
      ...
      document.querySelector("#bookmarks-button").addEventListener(
        "click",
        function () {
          const bookmarkData = appModel.getBookmarks().sort((a, b) =>
